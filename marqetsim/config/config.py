...
def _activate_dotenv(logger):
    env_local = Path.cwd() / ".env.local"
    env_prod = Path.cwd() / ".env"

    if env_local.exists():
        load_dotenv(env_local)
        logger.info(f"Using {env_local}")
    else:
        load_dotenv(env_prod)
        logger.info(f"Using {env_prod}")

    if all([env_local.exists(), env_prod.exists()]) is False:
        logger.warning(
            "No .env file found. Will use only system environment variables."
        )


def read_config_file(logger) -> configparser.ConfigParser:
    """read config file."""
    ...
    logger.info(f"Looking for default config on: {config_file_path}")
